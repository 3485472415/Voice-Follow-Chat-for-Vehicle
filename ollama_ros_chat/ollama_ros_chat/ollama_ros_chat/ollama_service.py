...
class OllamaChatNode(Node):
    def __init__(self):
        ...
        self.conversation_history = [
            {"role": "system", "content": "你是小薇，一个语音助手。回答简洁高效，不超过十二句话,始终默认用中文回答，除非要求使用其他语言"}]
    ...
    def handle_chat_request(self, request, response):
        try:
            ...
            #xgd
            is_intent = "[INTENT]" in user_message    # ← 加
            if is_intent:                              # ← 加
                user_message = user_message.replace("[INTENT]", "")  # ← 加
        # ... 后面完全不变 ...
            #xgd
            ...
            if reply:
                ...
                if is_intent:                          # ← 加
                    self.conversation_history = self.conversation_history[:-2]  # ← 加，删掉刚追加的两条
